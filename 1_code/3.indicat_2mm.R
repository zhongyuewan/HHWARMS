...
countTable.d1 <- as.data.frame(countTable[,])
names(countTable.d1)[1] <- 'taxa'



freqTable.d4 <- as.data.frame(countTable.d1%>% group_by(taxa) %>% summarise(across(1:18,sum)))    
...
set.seed(123)
  simper_result <- simper(t(freqTable), group = metadata$coral)
...
dataSIM.sig.2percent <- dataSIM.sig %>% filter(.[,10]>0.02)
